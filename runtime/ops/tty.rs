...
#[cfg(windows)]
const RAW_MODE_MASK: DWORD =
    wincon::ENABLE_LINE_INPUT | wincon::ENABLE_ECHO_INPUT | wincon::ENABLE_PROCESSED_INPUT;

#[cfg(windows)]
fn get_windows_handle(f: &std::fs::File) -> Result<std::os::windows::io::RawHandle, AnyError> {
...
fn op_set_raw(state: &mut OpState, args: SetRawArgs) -> Result<(), AnyError> {
    ...
    #[cfg(unix)]
    {
        ...
        let (fs_file, meta) = resource.fs_file.as_ref().ok_or_else(resource_unavailable)?;
...
#[op]
fn op_console_size(state: &mut OpState, rid: ResourceId) -> Result<ConsoleSize, AnyError> {
