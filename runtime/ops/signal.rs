...
#[cfg(unix)]
#[op]
fn op_signal_bind(state: &mut OpState, sig: String) -> Result<ResourceId, AnyError> {
...
#[cfg(unix)]
#[op]
async fn op_signal_poll(state: Rc<RefCell<OpState>>, rid: ResourceId) -> Result<bool, AnyError> {
    let resource = state
        .borrow_mut()
        .resource_table
